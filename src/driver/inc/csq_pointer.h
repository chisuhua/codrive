...
public:
    ...
    // schi I change _acc to pointer
    // accelerator* _acc;       ///< Accelerator where allocation is physically located.
    accelerator* _acc;       ///< Accelerator where allocation is physically located.
    ...
    AmPointerInfo(void *hostPointer, void *devicePointer, void* unalignedDevicePointer, int sizeBytes, accelerator* acc,  bool isInDeviceMem=false, bool isAmManaged=false) :
    ...
    AmPointerInfo(void *hostPointer, void *devicePointer, void* unalignedDevicePointer, std::size_t sizeBytes, accelerator* acc,  bool isInDeviceMem=false, bool isAmManaged=false) :
        _hostPointer(hostPointer),
        _devicePointer(devicePointer),
        _unalignedDevicePointer(unalignedDevicePointer),
        _sizeBytes(sizeBytes),
...
auto_voidp am_aligned_alloc(std::size_t size, accelerator &acc, unsigned flags, std::size_t alignment = 0);
 ...
 * @see am_free, am_copy
 */
auto_voidp am_alloc(std::size_t size, accelerator &acc, unsigned flags);
 ...
 */
am_status_t am_copy(void*  dst, const void*  src, std::size_t size, uint8_t dir) __attribute__ (( deprecated ("use accelerator_view::copy instead (and note src/dst order reversal)" ))) ;
...
void am_memtracker_update_peers(const accelerator &acc, int peerCnt, hsa_agent_s *agents);
 ...
 */
am_status_t am_memory_host_lock(accelerator &ac, void *hostPtr, std::size_t size, accelerator *visibleAc, std::size_t numVisibleAc);
