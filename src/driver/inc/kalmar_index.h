...
class __index_leaf {
    ...
    int dummy;
public:
    explicit __index_leaf(int __t)  : __idx(__t) {}

    __index_leaf& operator=(const int __t)  {
        __idx = __t;
        return *this;
    }
    __index_leaf& operator+=(const int __t)  {
    ...
    __index_leaf& operator%=(const int __t)  {
        ...
        return *this;
          ...
          int& get()        { return __idx; }
    const int& get() const  { return __idx; }
};
...
struct index_impl<__indices<N...> > : public __index_leaf<N>...  {
    index_impl()  : __index_leaf<N>(0)... {}

    template<class ..._Up>
        explicit index_impl(_Up... __u) 
        : __index_leaf<N>(__u)... {}

    index_impl(const index_impl& other) 
        : index_impl(static_cast<const __index_leaf<N>&>(other).get()...) {}

    index_impl(int component) 
        : __index_leaf<N>(component)... {}
    index_impl(int components[]) 
        : __index_leaf<N>(components[N])... {}
    index_impl(const int components[]) 
        : __index_leaf<N>(components[N])... {}

    template<class ..._Tp>
        inline void __swallow(_Tp...)  {}
    ...
    index_impl& operator=(const index_impl& __t)  {
    ...
    index_impl& operator+=(const int __t)  {
...
struct index_helper
{
    static inline void set(_Tp& now)  {
        now[N - 1] = amp_get_global_id(_Tp::rank - N);
        index_helper<N - 1, _Tp>::set(now);
    }
    static inline bool equal(const _Tp& _lhs, const _Tp& _rhs)  {
        return (_lhs[N - 1] == _rhs[N - 1]) &&
            (index_helper<N - 1, _Tp>::equal(_lhs, _rhs));
    }
    static inline int count_size(const _Tp& now)  {
...
struct amp_helper
{
    ...
    static bool inline contains(const _Tp1& idx, const _Tp2& ext,const _Tp2& ext2)  {
        return idx[N - 1] >= 0 && ext[N - 1] > 0 && (idx[N - 1] + ext[N - 1]) <= ext2[N - 1] &&
            amp_helper<N - 1, _Tp1, _Tp2>::contains(idx, ext,ext2);
    }

    static int inline flatten(const _Tp1& idx, const _Tp2& ext)  {
        return idx[N - 1] + ext[N - 1] * amp_helper<N - 1, _Tp1, _Tp2>::flatten(idx, ext);
    }
    static void inline minus(const _Tp1& idx, _Tp2& ext)  {
        ext.base_ -= idx.base_;
    }
};
...
public:
     ...
     */
    explicit index(int i0) 
        : base_(i0) {}

    template <typename ..._Tp>
        explicit index(_Tp ... __t) 
        : base_(__t...) {
     ...
     */
    explicit index(const int components[]) 
        : base_(components) {}
