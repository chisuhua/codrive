...
// namespace alias
// namespace fast_math is an alias of namespace Kalmar::fast_math
// schi namespace fast_math = Kalmar::fast_math;

// namespace precise_math is an alias of namespace Kalmar::precise_math
// schi namespace precise_math = Kalmar::precise_math;
 ...
 */
/* TODO schi it looks we don't use index
template <int N>
using index = csq::index<N>;
*/

// schi using runtime_exception = Kalmar::runtime_exception;
// schi using invalid_compute_domain = Kalmar::invalid_compute_domain;
// schi using accelerator_view_removed = Kalmar::accelerator_view_removed;
...
inline uint64_t get_system_ticks() {
    return csq::getContext()->getSystemTicks();
}
...
inline uint64_t get_tick_frequency() {
    return csq::getContext()->getSystemTickFrequency();
}
...
public:
     ...
     */
    void wait(csq::hcWaitMode mode = csq::hcWaitModeBlocked) const {
        if (this->valid()) {
        ...
        csq::getContext()->flushPrintfBuffer();
    }
...
private:
    ...
    std::shared_ptr<csq::AsyncOp> __asyncOp;

    completion_future(std::shared_ptr<csq::AsyncOp> event) : __amp_future(*(event->getFuture())), __asyncOp(event) {}

    completion_future(const std::shared_future<void> &__future)
        : __amp_future(__future), __thread_then(nullptr), __asyncOp(nullptr) {}

    friend class csq::DlaQueue;
...
public:
     ...
     */
    csq::queuing_mode get_queuing_mode() const { return pQueue->get_mode(); }

    /**
     * Returns the execution order of this accelerator_view.
     */
    csq::execute_order get_execute_order() const { return pQueue->get_execute_order(); }

    /**
     * Returns the queue priority of this accelerator_view.
     */
    csq::queue_priority get_queue_priority() const { return pQueue->get_queue_priority(); }
     ...
     *                     the expense of using one CPU core for active waiting.
     */
    void wait(csq::hcWaitMode waitMode = csq::hcWaitModeBlocked) { 
      pQueue->wait(waitMode); 
// schi      csq::getContext()->flushPrintfBuffer();
    }
     ...
     */
    completion_future create_marker(csq::memory_scope fence_scope=csq::system_scope) const;
     ...
     *         been completed.
     */
    completion_future create_blocking_marker(completion_future& dependent_future, csq::memory_scope fence_scope=csq::system_scope) const;
     ...
     *         been completed.
     */
    completion_future create_blocking_marker(std::initializer_list<completion_future> dependent_future_list, csq::memory_scope fence_scope=csq::system_scope) const;
...
//    completion_future create_blocking_marker(InputIterator first, InputIterator last, csq::memory_scope scope) const;

template<typename InputIterator>
completion_future
create_blocking_marker(InputIterator first, InputIterator last, csq::memory_scope scope) const {
...
//        pQueue->copy(src, dst, size_bytes);
     ...
     */
    void copy_ext(const void *src, void *dst, size_t size_bytes, hcCommandKind copyDir, const AmPointerInfo &srcInfo, const AmPointerInfo &dstInfo, const accelerator *copyAcc, bool forceUnpinnedCopy);


    // TODO - this form is deprecated, provided for use with older HIP runtimes.
    void copy_ext(const void *src, void *dst, size_t size_bytes, hcCommandKind copyDir, const AmPointerInfo &srcInfo, const AmPointerInfo &dstInfo, bool forceUnpinnedCopy) ;
    ...
    completion_future copy_async_ext(const void *src, void *dst, size_t size_bytes, 
                                     csq::hcCommandKind copyDir, const AmPointerInfo &srcInfo, const AmPointerInfo &dstInfo, 
    ...
    */
    ...
    void* get_hsa_am_system_region() {
        return pQueue->getDlaAMHostRegion();
    }
    ...
    void* get_hsa_am_finegrained_system_region() {
        return pQueue->getDlaCoherentAMHostRegion();
    }
    ...
    void* get_hsa_kernarg_region() {
        return pQueue->getDlaKernargRegion();
    }
    ...
    bool is_hsa_accelerator() {
        return true;   // TODO schi I assume all client use HSA queue
    ...
    void dispatch_hsa_kernel(const hsa_kernel_dispatch_packet_t *aql, 
                           ...
                           completion_future *cf=nullptr, const char *kernel_name = nullptr) 
    {
...
private:
    accelerator_view(std::shared_ptr<csq::Queue> pQueue) : pQueue(pQueue) {}
    std::shared_ptr<csq::Queue> pQueue;
...
public:
    ...
    explicit accelerator(const std::wstring& path)
        : pDev(csq::getContext()->getDevice(path)) {}
...
/*
    static accelerator_view get_auto_selection_view() {
        return csq::getContext()->auto_select();
    }
*/
    /**
     ...
     */
    accelerator_view create_view(csq::execute_order order = csq::execute_in_order, csq::queuing_mode mode = csq::queuing_mode_automatic, queue_priority priority = priority_normal);
    ...
    */
     ...
     */
    // schi std::wstring get_description() const { return pDev->get_description(); }
     ...
     * use.
     */
    // schi size_t get_dedicated_memory() const { return pDev->get_mem(); }
     ...
     */
    // schi bool get_supports_double_precision() const { return pDev->is_double(); }
     ...
     */
    // schi bool get_supports_limited_double_precision() const { return pDev->is_lim_double(); }
     ...
     */
    // schi bool get_supports_cpu_shared_memory() const { return pDev->is_unified(); }
...
*/
    ...
    std::vector<accelerator_view> get_all_views() {
        ...
        std::vector< std::shared_ptr<csq::Queue> > queues = pDev->get_all_queues();
        for (auto q : queues) {
            result.push_back(q);
        }
        return result;
    ...
    void memcpy_symbol(const char* symbolName, void* hostptr, size_t count, size_t offset = 0, csq::hcCommandKind kind = csq::hcMemcpyHostToDevice) {
    ...
    */
    ...
    csq::Device *get_dev_ptr() const { return pDev; }; 

private:
    accelerator(csq::Device* pDev) : pDev(pDev) {}
    friend class accelerator_view;
    csq::Device* pDev;
};
...
//        : array(extent<N>(e0)) { static_assert(N == 1, "illegal"); }
//    explicit array(int e0, int e1)
//        : array(extent<N>(e0, e1)) {}
...
//        : array(extent<N>(e0), accelerator(L"default").get_default_view(), accelerator_pointer) {}
//    explicit array(int e0, int e1, void* accelerator_pointer)
//        : array(extent<N>(e0, e1), accelerator(L"default").get_default_view(), accelerator_pointer) {}
//    explicit array(int e0, int e1, int e2, void* accelerator_pointer)
//        : array(extent<N>(e0, e1, e2), accelerator(L"default").get_default_view(), accelerator_pointer) {}
//
...
//        : array(extent<N>(e0), av, associated_av) {}
//    array(int e0, int e1, accelerator_view av, accelerator_view associated_av)
//        : array(extent<N>(e0, e1), av, associated_av) {}
...
//        return ptr[Kalmar::amp_helper<N, index<N>, extent<N>>::flatten(idx, extent)];
//    }
//    T& operator()(const index<N>& idx) __CPU__ __HC__ {
//        return (*this)[idx];
//    }
...
//#if __KALMAR_ACCELERATOR__ != 1
//        if ( !Kalmar::amp_helper<N, index<N>, extent<N>>::contains(origin,  ext ,this->extent) )
//            throw runtime_exception("errorMsg_throw", 0);
//#endif
...
//#if __KALMAR_ACCELERATOR__ != 1
//        if ( !Kalmar::amp_helper<N, index<N>, extent<N>>::contains(idx, this->extent ) )
//            throw runtime_exception("errorMsg_throw", 0);
//#endif
//        array_view<T, N> av(*this);
...
//        return section(index<2>(i0, i1), extent<2>(e0, e1));
//    }
...
//        return section(index<3>(i0, i1, i2), extent<3>(e0, e1, e2));
//    }
...
//            : array_view(extent<N>(e0), src) {}
//    template <typename Container, class = typename std::enable_if<__is_container<Container>::value>::type>
//        array_view(int e0, int e1, Container& src)
//            : array_view(extent<N>(e0, e1), src) {}
//    template <typename Container, class = typename std::enable_if<__is_container<Container>::value>::type>
...
//     *                     this array_view.
//     */
//    explicit array_view(int e0) : array_view(extent<N>(e0)) {}
//    explicit array_view(int e0, int e1)
//        : array_view(extent<N>(e0, e1)) {}
...
//        return ptr[Kalmar::amp_helper<N, index<N>, extent<N>>::flatten(idx + index_base, extent_base)];
//    }
//
//    T& operator()(const index<N>& idx) const __CPU__ __HC__ {
//        return (*this)[idx];
...
//#if __KALMAR_ACCELERATOR__ != 1
//        if ( !Kalmar::amp_helper<N, index<N>, extent<N>>::contains(idx, ext,this->extent ) )
//            throw runtime_exception("errorMsg_throw", 0);
//#endif
...
//        extent<N> ext(extent);
//        Kalmar::amp_helper<N, index<N>, extent<N>>::minus(idx, ext);
...
//    array_view(const acc_buffer_t& cache, const extent<N>& ext,
//               int offset) __CPU__ __HC__
//        : cache(cache), extent(ext), extent_base(ext), offset(offset) {}
//
//    // used by section and projection
//    array_view(const acc_buffer_t& cache, const extent<N>& ext_now,
 ...
 *                  array or array_view or within a
...
#else
/* TODO schi do we need below atomic?
 ...
 */
// TODO schi atomic extern "C" unsigned int __atomic_wrapinc(unsigned int* address, unsigned int val) __HC__;
 ...
 */
// TODO schi atomic extern "C" unsigned int __atomic_wrapdec(unsigned int* address, unsigned int val) __HC__;
...
// schi #pragma clang diagnostic push
// schi #pragma clang diagnostic ignored "-Wreturn-type"
// schi #pragma clang diagnostic ignored "-Wunused-variable"
