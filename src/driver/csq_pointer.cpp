...
struct AmMemoryRange
{
    ...
    AmMemoryRange(const void *basePointer, std::size_t sizeBytes) : _basePointer(basePointer), _endPointer((const unsigned char *)basePointer + sizeBytes - 1){};
};
...
void printShortPointerInfo(std::ostream &os, const AmPointerInfo &ap)
{
    using namespace std;
...
void printhcsPointerInfo(std::ostream &os, const void *ptr)
{
    ...
    if (hsa_status == SUCCESS)
    {
        ...
        for (uint32_t i = 0; i < peerAgentCnt; i++)
        {
            ...
            //<< "(" << accelerator::get_seqnum_from_agent(peerAgents[i]) << ")" << std::dec;
        }
...
std::ostream &operator<<(std::ostream &os, const AmPointerInfo &ap)
{
    ...
    printhcsPointerInfo(os, ap._isInDeviceMem ? ap._devicePointer : ap._hostPointer);
    return os;
}
...
{
    typedef std::map<AmMemoryRange, AmPointerInfo, AmMemoryRangeCompare> MapTrackerType;

  public:
    void insert(void *pointer, AmPointerInfo &p);
    int remove(void *pointer);

    MapTrackerType::iterator find(const void *hostPtr);

    MapTrackerType::iterator readerLockBegin()
    {
        _mutex.lock();
    ...
    std::size_t reset(const accelerator &acc);
    void update_peers(const accelerator &acc, int peerCnt, device_t *peerAgents);
...
//---
void AmPointerTracker::insert(void *pointer, AmPointerInfo &p)
{
...
std::size_t AmPointerTracker::reset(const accelerator &acc)
{
    std::lock_guard<std::mutex> l(_mutex);
    mprintf("reset: \n");
...
// Remove all tracked locations, and free the associated memory (if the range was originally allocated by AM).
// Returns count of ranges removed.
void AmPointerTracker::update_peers(const accelerator &acc, int peerCnt, device_t *peerAgents)
{
    std::lock_guard<std::mutex> l(_mutex);
...
static AmPointerTracker g_amPointerTracker; // Track all am pointer allocations.

// Allocate accelerator memory, return NULL if memory could not be allocated:
auto_voidp am_aligned_alloc(std::size_t sizeBytes, accelerator &acc, unsigned flags, std::size_t alignment)
{
    void *ptr = NULL;
    ...
    if (sizeBytes != 0)
    {
        ...
        // device_t *hsa_agent = static_cast<hsa_agent_t*> (acc.get_default_view().get_hsa_agent());
        ...
        if (alloc_region && (int)(alloc_region->handle) != -1)
        {
            sizeBytes = alignment != 0 ? sizeBytes + alignment : sizeBytes;
            status_t s1 = hsa_amd_memory_pool_allocate(*alloc_region, sizeBytes, 0, &ptr);
            ...
            else
            {
                if (flags & (amHostPinned | amHostCoherent))
                {
                    ...
                    {
                        AmPointerInfo ampi(ptr /*hostPointer*/, ptr /*devicePointer*/, unalignedPtr, sizeBytes, &acc, false /*isDevice*/, true /*isAMManaged*/);
                ...
                {
                    AmPointerInfo ampi(NULL /*hostPointer*/, ptr /*devicePointer*/, unalignedPtr, sizeBytes, &acc, true /*isDevice*/, true /*isAMManaged*/);
                    g_amPointerTracker.insert(unalignedPtr, ampi);
                }
            }
        }
...
auto_voidp am_alloc(std::size_t sizeBytes, accelerator &acc, unsigned flags)
{
...
am_status_t am_copy(void *dst, const void *src, std::size_t sizeBytes, uint8_t dir)
{
    am_status_t am_status = AM_ERROR_MISC;
    status_t err = hsa_memory_copy(dst, src, sizeBytes);
...
am_status_t am_memtracker_getinfo(AmPointerInfo *info, const void *ptr)
{
...
am_status_t am_memtracker_add(void *ptr, AmPointerInfo &info)
{
...
void am_memtracker_print(void *targetAddress)
{
    ...
    if (targetAddress)
    {
        for (auto iter = g_amPointerTracker.readerLockBegin(); iter != g_amPointerTracker.end(); iter++)
        {
            ...
            {
                if ((targetAddressP < basePointer) && (basePointer - targetAddressP < (int)beforeD))
                {
                    beforeD = (basePointer - targetAddressP);
                    closestBefore = iter;
                }
                if ((targetAddressP > endPointer) && (targetAddressP - endPointer < (int)afterD))
                {
    ...
    else
    {
        ...
        for (auto iter = g_amPointerTracker.readerLockBegin(); iter != g_amPointerTracker.end(); iter++)
        {
            ...
            printhcsPointerInfo(os, iter->first._basePointer);
            os << "\n";
        }
    }
...
//---
void am_memtracker_sizeinfo(const accelerator &acc, std::size_t *deviceMemSize, std::size_t *hostMemSize, std::size_t *userMemSize)
{
...
//---
std::size_t am_memtracker_reset(const accelerator &acc)
{
    return g_amPointerTracker.reset(acc);
}

void am_memtracker_update_peers(const accelerator &acc, int peerCnt, device_t *peerAgents)
{
    return g_amPointerTracker.update_peers(acc, peerCnt, peerAgents);
}

am_status_t am_map_to_peers(void *ptr, std::size_t num_peer, const accelerator *peers)
{
    // check input
    ...
    std::vector<device_t> agents{accelerator::get_all().size()};

    int peer_count = 0;

    for (auto i = 0; i < (int)num_peer; i++)
    {
        ...
        device_t *agent = static_cast<hsa_agent_t *>(a.get_hsa_agent());

        if (agent)
        {

            hsa_amd_memory_pool_access_t access;
            status_t status = hsa_amd_agent_memory_pool_get_info(*agent, *pool, HSA_AMD_AGENT_MEMORY_POOL_INFO_ACCESS, &access);
            if (SUCCESS != status)
                return AM_ERROR_MISC;
    ...
    if (peer_count)
    {
        status_t status = hsa_amd_agents_allow_access(peer_count, agents.data(), NULL, ptr);
        return status == SUCCESS ? AM_SUCCESS : AM_ERROR_MISC;
    }

    return AM_SUCCESS;
}

am_status_t am_memory_host_lock(accelerator &ac, void *hostPtr, std::size_t size, accelerator *visible_ac, std::size_t num_visible_ac)
{
    am_status_t am_status = AM_ERROR_MISC;
    void *devPtr;
    std::vector<device_t> agents;
    for (int i = 0; i < (int)num_visible_ac; i++)
    {
        agents.push_back(*static_cast<device_t *>(visible_ac[i].get_hsa_agent()));
    }
    status_t hsa_status = hsa_amd_memory_lock(hostPtr, size, &agents[0], num_visible_ac, &devPtr);
    if (hsa_status == SUCCESS)
    {
        AmPointerInfo ampi(hostPtr, devPtr, devPtr, size, &ac, false, false);
...
am_status_t am_memory_host_unlock(accelerator &ac, void *hostPtr)
{
    am_status_t am_status = AM_ERROR_MISC;
    AmPointerInfo amPointerInfo(NULL, NULL, NULL, 0, &ac, 0, 0);
    am_status = am_memtracker_getinfo(&amPointerInfo, hostPtr);
    if (am_status == AM_SUCCESS)
    {
        status_t hsa_status = hsa_amd_memory_unlock(hostPtr);
        if (hsa_status == SUCCESS)
        {
...
auto_voidp am_alloc_host_coherent(size_t size)
{
    ...
    void *buffer = am_alloc(size, acc, amHostCoherent);
    return buffer;
}
