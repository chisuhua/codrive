		...
		help(help)
{
	...
	utils::StringTokenize(name, tokens);
...
void CommandLineOptionInt32::Read(std::deque<std::string> &arguments)
{
	...
	*variable = StringToInt(argument, error);
	...
	// Check valid value
	if (error)
		throw CommandLine::Error(utils::fmt("Invalid value for option "
				"'%s': %s", getName().c_str(),
...
CommandLine *CommandLine::getInstance()
{
	...
	// Create instance
	instance = utils::make_unique<CommandLine>();
	return instance.get();
}
...
CommandLine::CommandLine()
{
	...
	categories.emplace_back(utils::make_unique<CommandLineCategory>
			("default"));
...
void CommandLine::Register(std::unique_ptr<CommandLineOption> &&option)
{
	...
	if (!StringPrefix(option->getName(), "-"))
		throw utils::Panic(utils::fmt("Option name must start with a "
				"dash (%s)", option->getName().c_str()));
	
	// Options with one single dash must have one letter only
	if (!StringPrefix(option->getName(), "--") &&
			option->getName().length() != 2)
		throw utils::Panic(utils::fmt("Option with one dash must be one "
				"single character (%s)",
	...
	if (option_table.find(option->getName()) != option_table.end())
		throw utils::Panic(utils::fmt("Option name already registered "
...
void CommandLine::setIncompatible(const std::string &name)
{
	...
	if (it == option_table.end())
		throw utils::Panic(utils::fmt("Option no registered (%s)",
				name.c_str()));
...
void CommandLine::Process(int argc, char **argv, bool options_anywhere)
{
	...
	while (arguments.size() > 0)
	{
		...
		if (it == option_table.end())
			throw Error(utils::fmt("Invalid option: %s\n%s",
					argument.c_str(),
					error_message.c_str()));
		...
		if (attached_argument && option->getNumArguments() != 1)
			throw Error(utils::fmt("Option '%s' expects %d "
					"argument(s)",
					option->getName().c_str(),
		...
		if (options.find(argument) != options.end())
			throw Error(utils::fmt("Multiple occurrences of "
					"option '%s'",
					option->getName().c_str()));
	...
	if (options.size() > 1)
	{
		for (auto &it : options)
		{
			...
			if (option->isIncompatible())
				throw Error(utils::fmt("Option '%s' is "
