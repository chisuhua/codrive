...
extern bool id_in_array(uint32_t id, uint32_t *ids_array, uint32_t ids_array_size);
extern bool topology_setup_is_dgpu_param(HsaCoreProperties *props);
extern inline uint32_t get_num_sysfs_nodes(void);


/* Reserved aperture type managed by its own address allocator */
static void *reserved_aperture_allocate_aligned(Aperture *aper,
						void *addr,
						uint64_t size, uint64_t align);
static void reserved_aperture_release(Aperture *aper,
				      void *addr, uint64_t size);

static ApertureOps reserved_aperture_ops = {
...
static void *mmap_aperture_allocate_aligned(Aperture *aper,
					    void *addr,
					    uint64_t size, uint64_t align);
static void mmap_aperture_release(Aperture *aper,
				  void *addr, uint64_t size);
static  ApertureOps mmap_aperture_ops = {
...
KernelMutex Aperture::mm_lock_;

gpu_mem_t::gpu_mem_t() {
    scratch_physical =  new Aperture(0, 0, &reserved_aperture_ops);
    gpuvm_aperture =  new Aperture(0, 0, &reserved_aperture_ops);
}
...
class svm_t {
    public:
        svm_t()
            : apertures({{0, 0, &mmap_aperture_ops}, {0,0, &mmap_aperture_ops}})
 ...
 */
static Aperture cpuvm_aperture {nullptr, nullptr, &reserved_aperture_ops};
				      ...
				      uint64_t bytes)
{
	...
	bytes = aper->vm_align_area_size(bytes);

    bool result = aper->allocator->free(address, bytes);
	if (!result)
		return;
					    ...
					    uint64_t align)
{
	...
	start = address ? address :
		(void *)(ALIGN_UP((uint64_t)aper->base_, align) + offset);

    start = aper->allocator->alloc(start, bytes, address ? true: false);
					    ...
					    uint64_t size, uint64_t align)
{
	 ...
	 * pages on both sides
	 */
	guard_size = (uint64_t)aper->guard_pages_ * PAGE_SIZE;
	...
	if (aligned_addr < aper->base_ ||
	    VOID_PTR_ADD(aligned_addr, size - 1) > aper->limit_) {
		pr_err("mmap returned %p, out of range %p-%p\n", aligned_addr,
		       aper->base_, aper->limit_);
...
// fmm_get_aperture
static Aperture *mm_get_aperture(HsaApertureInfo info)
{
	switch (info.type) {
...
// *fmm_is_scratch_aperture
static Aperture *mm_is_scratch_aperture(const void *address)
{
	uint32_t i;
	...
	for (i = 0; i < gpu_mem_count; i++) {
		...
		if ((address >= gpu_mem[i].scratch_physical->base_) &&
			(address <= gpu_mem[i].scratch_physical->limit_))
...
// fmm_find_aperture
static Aperture *mm_find_aperture(const void *address,
						HsaApertureInfo *info)
{
	...
	if (is_dgpu) {
		if (address >= svm.dgpu_aperture->base_ && address <= svm.dgpu_aperture->limit_) {
			aperture = mm_is_scratch_aperture(address);
			if (!aperture) {
				aperture = svm.dgpu_aperture;
				_info.type = HSA_APERTURE_DGPU;
			}
		} else if (address >= svm.dgpu_alt_aperture->base_ && address <= svm.dgpu_alt_aperture->limit_) {
		...
		if (address >= svm.dgpu_aperture->base_ && address <= svm.dgpu_aperture->limit_) {
			...
			/* gpuvm_aperture */
			for (i = 0; i < gpu_mem_count; i++) {
				if ((address >= gpu_mem[i].gpuvm_aperture->base_) &&
					(address <= gpu_mem[i].gpuvm_aperture->limit_)) {
 ...
 */
static MemObj *vm_find_object(const void *addr, uint64_t size,
				   Aperture **out_aper)
{
	Aperture *aper = NULL;
	...
	for (i = 0; i < gpu_mem_count; i++)
		if (gpu_mem[i].gpu_id != NON_VALID_GPU_ID &&
		    addr >= gpu_mem[i].gpuvm_aperture->base_ &&
		    addr <= gpu_mem[i].gpuvm_aperture->limit_) {
	...
	if (!aper) {
		...
		if ((addr >= svm.dgpu_aperture->base_) &&
		    (addr <= svm.dgpu_aperture->limit_))
			aper = svm.dgpu_aperture;
		else if ((addr >= svm.dgpu_alt_aperture->base_) &&
			 (addr <= svm.dgpu_alt_aperture->limit_))
...
static uint8_t mm_check_user_memory(const void *addr, uint64_t size)
{
	 ...
	 * to debug.
	 */
	for (; ptr < end; ptr = (volatile const uint8_t *)PAGE_ALIGN_UP(ptr + 1))
		sum += *ptr;
...
static void mm_release_scratch(uint32_t gpu_id)
{
	int32_t gpu_mem_id;
	uint64_t size;
	MemObj *obj;
	...
	if (is_dgpu) {
        ...
        {
            ScopedAcquire<KernelMutex> lock(&svm.dgpu_aperture->mm_lock_);
		    svm.dgpu_aperture->aperture_release_area(
				      gpu_mem[gpu_mem_id].scratch_physical->base_,
				      size);
        }
	} else
		/* release address space */
		munmap(gpu_mem[gpu_mem_id].scratch_physical->base_, size);

	/* invalidate scratch backing aperture */
	gpu_mem[gpu_mem_id].scratch_physical->base_ = NULL;
	gpu_mem[gpu_mem_id].scratch_physical->limit_ = NULL;
}

// mm_translate_hsa_to_ioc_flags(HsaMemFlags flags)
static uint32_t mm_translate_hsa_to_ioc_flags(HsaMemFlags flags)
{
...
void *mm_allocate_scratch(uint32_t gpu_id, void *address, uint64_t MemorySizeInBytes)
{
	...
	if (gpu_mem_id >= 0) {
        ...
        ScopedAcquire<KernelMutex> lock(&svm.dgpu_aperture->mm_lock_);
		mem = svm.dgpu_aperture->aperture_allocate_area_aligned(
...
void *mm_allocate_doorbell(uint32_t gpu_id, uint64_t MemorySizeInBytes,
...
static void *mm_allocate_host_cpu(void *address, uint64_t MemorySizeInBytes,
				HsaMemFlags flags)
{
	void *mem = NULL;
	MemObj *vm_obj;
...
static void *mm_allocate_host_gpu(uint32_t node_id, void *address,
				   uint64_t MemorySizeInBytes, HsaMemFlags flags)
{
    ...
    MAKE_SCOPE_GUARD([&]() {
        if (mem && !vm_obj) {
            ScopedAcquire<KernelMutex> lock(&aperture->mm_lock_);
            aperture->aperture_release_area(mem, size);
        }
    });
...
void *mm_allocate_host(uint32_t node_id, void *address,
			uint64_t MemorySizeInBytes, HsaMemFlags flags)
{
	if (is_dgpu)
...
device_status_t mm_release(void *address)
{
	...
	for (i = 0; i < gpu_mem_count; i++)
		if (gpu_mem[i].gpu_id != NON_VALID_GPU_ID &&
		    address >= gpu_mem[i].scratch_physical->base_ &&
		    address <= gpu_mem[i].scratch_physical->limit_) {
			mm_release_scratch(gpu_mem[i].gpu_id);
...
/*
static int mm_set_memory_policy(uint32_t gpu_id, int default_policy, int alt_policy,
				 uintptr_t alt_base, uint64_t alt_size)
{
	struct ioctl_set_memory_policy_args args = {0};
	...
	return cmd_set_memory_policy(&args);
}
*/
...
static uint32_t get_vm_alignment(uint32_t device_id)
{
	...
	return std::max(PAGE_SIZE, page_size);
}
// TODO move to cmd_api
...
static device_status_t acquire_vm(uint32_t gpu_id, int fd)
{
...
static device_status_t init_mmap_apertures(uint64_t base, uint64_t limit,
					 uint32_t align, uint32_t guard_pages)
{
	...
	svm.apertures[SVM_DEFAULT].base_  = (void *)base;
	svm.apertures[SVM_DEFAULT].limit_ = (void *)limit;
	svm.apertures[SVM_DEFAULT].align_ = align;
	svm.apertures[SVM_DEFAULT].guard_pages_ = guard_pages;
	svm.apertures[SVM_DEFAULT].is_cpu_accessible = true;
	svm.apertures[SVM_DEFAULT].ops = &mmap_aperture_ops;

	svm.apertures[SVM_DEFAULT].init_allocator();

	svm.apertures[SVM_COHERENT].base_ = svm.apertures[SVM_COHERENT].limit_ = NULL;

	svm.apertures[SVM_COHERENT].init_allocator();

	/* Try to allocate one page. If it fails, we'll fall back to
	 * managing our own reserved address range.
	 */
	addr = svm.apertures[SVM_DEFAULT].aperture_allocate_area(NULL, PAGE_SIZE);
	if (addr) {
		svm.apertures[SVM_DEFAULT].aperture_release_area(addr, PAGE_SIZE);

		svm.dgpu_aperture = svm.dgpu_alt_aperture = &svm.apertures[SVM_DEFAULT];
		pr_info("Initialized unreserved SVM apertures: %p - %p\n",
			svm.apertures[SVM_DEFAULT].base_,
			svm.apertures[SVM_DEFAULT].limit_);
	} else {
...
static device_status_t init_svm_apertures(uint64_t base, uint64_t limit,
					uint32_t align, uint32_t guard_pages)
{
	...
	svm.apertures[SVM_DEFAULT].base_  = dgpu_shared_aperture_base  = ret_addr;
	svm.apertures[SVM_DEFAULT].limit_ = dgpu_shared_aperture_limit = (void *)limit;
	svm.apertures[SVM_DEFAULT].align_ = align;
	svm.apertures[SVM_DEFAULT].guard_pages_ = guard_pages;
	svm.apertures[SVM_DEFAULT].is_cpu_accessible = true;
	 ...
	 */
	alt_base = (HSAuint64)svm.apertures[SVM_DEFAULT].base_;
	alt_size = (VOID_PTRS_SUB(svm.apertures[SVM_DEFAULT].limit_,
				  svm.apertures[SVM_DEFAULT].base_) + 1) >> 2;
	alt_base = (alt_base + 0xffff) & ~0xffffULL;
	alt_size = (alt_size + 0xffff) & ~0xffffULL;
	svm.apertures[SVM_COHERENT].base_ = (void *)alt_base;
	svm.apertures[SVM_COHERENT].limit_ = (void *)(alt_base + alt_size - 1);
	svm.apertures[SVM_COHERENT].align_ = align;
	svm.apertures[SVM_COHERENT].guard_pages_ = guard_pages;
	svm.apertures[SVM_COHERENT].is_cpu_accessible = true;
	svm.apertures[SVM_COHERENT].ops = &reserved_aperture_ops;

	svm.apertures[SVM_DEFAULT].base_ = VOID_PTR_ADD(svm.apertures[SVM_COHERENT].limit_, 1);

	svm.apertures[SVM_DEFAULT].init_allocator();
	svm.apertures[SVM_COHERENT].init_allocator();

	pr_info("SVM alt (coherent): %12p - %12p\n",
		svm.apertures[SVM_COHERENT].base_, svm.apertures[SVM_COHERENT].limit_);
	pr_info("SVM (non-coherent): %12p - %12p\n",
		svm.apertures[SVM_DEFAULT].base_, svm.apertures[SVM_DEFAULT].limit_);
...
{
	void *mem;
	Aperture *aperture = svm.dgpu_alt_aperture;
	uint32_t ioc_flags;
	MemObj *vm_obj = NULL;
	HsaMemFlags flags;
	void *ret;
...
static void release_mmio(void)
{
	...
	for (gpu_mem_id = 0; (uint32_t)gpu_mem_id < gpu_mem_count; gpu_mem_id++) {
		if (!gpu_mem[gpu_mem_id].mmio_aperture.base)
			continue;
		mm_unmap_from_gpu(gpu_mem[gpu_mem_id].mmio_aperture.base);
		munmap(gpu_mem[gpu_mem_id].mmio_aperture.base, PAGE_SIZE);
		mm_release(gpu_mem[gpu_mem_id].mmio_aperture.base);
	}
}
...
device_status_t mm_init_process_apertures(unsigned int NumNodes, node_props_t *node_props)
{
	...
	for (i = 0; i < NumNodes; i++) {
		...
		if (gpu_id != 0) {
			...
			gpu_mem[gpu_mem_count].scratch_physical->align_ = PAGE_SIZE;
			...
			gpu_mem[gpu_mem_count].gpuvm_aperture->align_ = get_vm_alignment(props->DeviceId);
			gpu_mem[gpu_mem_count].gpuvm_aperture->guard_pages_ = guardPages;
    ...
    */
	...
	if (num_of_sysfs_nodes > 0) {
		all_gpu_id_array = (uint32_t*)malloc(sizeof(uint32_t) * gpu_mem_count);
		if (!all_gpu_id_array) {
			ret = DEVICE_STATUS_NO_MEMORY;
			goto get_aperture_ioctl_failed;
		}
	...
	for (i = 0 ; i < num_of_sysfs_nodes ; i++) {
		...
		gpu_mem[gpu_mem_id].scratch_aperture.base = PORT_UINT64_TO_VPTR(process_apertures[i].scratch_base);
		gpu_mem[gpu_mem_id].scratch_aperture.limit = PORT_UINT64_TO_VPTR(process_apertures[i].scratch_limit);
		...
		if (IS_CANONICAL_ADDR(process_apertures[i].gpuvm_limit)) {
			...
			gpu_mem[gpu_mem_id].scratch_physical->align_ = vm_alignment;

			/* Non-canonical per-ASIC GPUVM aperture does
			 * not exist on dGPUs in GPUVM64 address mode
			 */
			gpu_mem[gpu_mem_id].gpuvm_aperture->base_ = NULL;
			gpu_mem[gpu_mem_id].gpuvm_aperture->limit_ = NULL;
			...
			gpu_mem[gpu_mem_id].gpuvm_aperture->base_ =
				PORT_UINT64_TO_VPTR(process_apertures[i].gpuvm_base);
			gpu_mem[gpu_mem_id].gpuvm_aperture->limit_ =
			 ...
			 */
			gpu_mem[gpu_mem_id].gpuvm_aperture->aperture_allocate_area(
				NULL,
				gpu_mem[gpu_mem_id].gpuvm_aperture->align_);
		}
	...
	if (svm_limit) {
		...
		for (i = 0 ; i < num_of_sysfs_nodes ; i++) {
			...
			alt_base = (uintptr_t)svm.dgpu_alt_aperture->base_;
			alt_size = VOID_PTRS_SUB(svm.dgpu_alt_aperture->limit_,
				svm.dgpu_alt_aperture->base_) + 1;
            /*
			...
			uint64_t *aperture_base, uint64_t *aperture_limit)
{
	...
	case FMM_GPUVM:
		if (aperture_is_valid(gpu_mem[slot].gpuvm_aperture->base_,
			gpu_mem[slot].gpuvm_aperture->limit_)) {
			*aperture_base = PORT_VPTR_TO_UINT64(gpu_mem[slot].gpuvm_aperture->base_);
			*aperture_limit = PORT_VPTR_TO_UINT64(gpu_mem[slot].gpuvm_aperture->limit_);
	...
	case FMM_SVM:
		 ...
		 */
		if (aperture_is_valid(svm.dgpu_alt_aperture->base_,
				      svm.dgpu_aperture->limit_)) {
			*aperture_base = PORT_VPTR_TO_UINT64(svm.dgpu_alt_aperture->base_);
			*aperture_limit = PORT_VPTR_TO_UINT64(svm.dgpu_aperture->limit_);
			err = DEVICE_STATUS_SUCCESS;
		}
		break;
	default:
...
int mm_map_to_gpu(void *address, uint64_t size, uint64_t *gpuvm_address)
{
	...
	for (i = 0; i < gpu_mem_count; i++)
		if (gpu_mem[i].gpu_id != NON_VALID_GPU_ID &&
		    address >= gpu_mem[i].scratch_physical->base_ &&
		    address <= gpu_mem[i].scratch_physical->limit_)
			return	gpu_mem[i].scratch_physical->mm_map_to_gpu_scratch(
                            gpu_mem[i].gpu_id,
							address, size);
	...
	if (aperture == &cpuvm_aperture) {
		...
		ret = svm.dgpu_aperture->mm_map_to_gpu_userptr(address, size, gpuvm_address, object);
	} else {
...
int mm_unmap_from_gpu(void *address)
{
	...
	for (i = 0; i < gpu_mem_count; i++)
		if (gpu_mem[i].gpu_id != NON_VALID_GPU_ID &&
		    address >= gpu_mem[i].scratch_physical->base_ &&
		    address <= gpu_mem[i].scratch_physical->limit_)
		    return gpu_mem[i].scratch_physical->mm_unmap_from_gpu_scratch(
                            gpu_mem[i].gpu_id, address);
...
bool mm_get_handle(void *address, uint64_t *handle)
{
	...
	if (!aperture) {
		if ((address >= svm.dgpu_aperture->base_) &&
			(address <= svm.dgpu_aperture->limit_)) {
			aperture = svm.dgpu_aperture;
		} else if ((address >= svm.dgpu_alt_aperture->base_) &&
			(address <= svm.dgpu_alt_aperture->limit_)) {
			aperture = svm.dgpu_alt_aperture;
		}
	}
...
static device_status_t mm_register_user_memory(void *addr, HSAuint64 size,
				  MemObj **obj_ret, bool coarse_grain)
{
	Aperture *aperture = svm.dgpu_aperture;
...
device_status_t mm_register_graphics_handle(HSAuint64 GraphicsResourceHandle,
					   ...
					   uint32_t gpu_id_array_size)
{
    ...
    MAKE_SCOPE_GUARD([&]() {
        if (r)
	        aperture->aperture_release_area(mem, infoArgs.size);
    });
...
err_free_mem:
	aperture->aperture_release_area(reservedMem, (SizeInPages << PAGE_SHIFT));
	// pthread_mutex_unlock(&aperture->fmm_mutex);
err_free_buffer:
	freeArgs.handle = importArgs.handle;
	cmd_free_memory(&freeArgs);
		...
		uint64_t *gpuvm_address)
{
    ...
    //FIXME
	if (object->userptr) {
		retcode = svm.dgpu_aperture->mm_map_to_gpu_userptr(address, size,
...
#if 0
device_status_t fmm_get_mem_info(const void *address, HsaPointerInfo *info)
{
	device_status_t ret = DEVICE_STATUS_SUCCESS;
	uint32_t i;
	Aperture *aperture;
...
void mm_clear_all_mem(void)
{
	...
	svm.apertures[SVM_DEFAULT].mm_clear_aperture();
	svm.apertures[SVM_COHERENT].mm_clear_aperture();
	...
	for (i = 0; i < gpu_mem_count; i++) {
		gpu_mem[i].gpuvm_aperture->mm_clear_aperture();
		gpu_mem[i].scratch_physical->mm_clear_aperture();
	}
