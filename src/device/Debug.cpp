...
device_status_t Debug::init_device_debugging_memory(unsigned int NumNodes)
{
	unsigned int i;
...
void Debug::destroy_device_debugging_memory(void)
{
	if (is_device_debugged) {
...
device_status_t Debug::DbgRegister(uint32_t NodeId)
{
	...
	result = device_->get_topo()->validate_nodeid(NodeId, &gpu_id);
...
device_status_t Debug::DbgUnregister(uint32_t NodeId)
{
	uint32_t gpu_id;
	device_status_t result;
...
device_status_t Debug::DbgWavefrontControl(uint32_t NodeId,
...
device_status_t Debug::DbgAddressWatch(uint32_t NodeId,
...
int Debug::debug_get_reg_status(uint32_t node_id, bool *is_debugged)
{
	*is_debugged = NULL;
	if (!is_device_debugged)
		return -1;
...
device_status_t Debug::debug_trap(uint32_t NodeId,
				uint32_t op,
				uint32_t data1,
				uint32_t data2,
				uint32_t data3,
...
device_status_t Debug::EnableDebugTrapWithPollFd(uint32_t   NodeId,
							HSA_QUEUEID QueueId,
							HSAint32 *PollFd) //OUT
{
	device_status_t result;
...
device_status_t Debug::EnableDebugTrap(uint32_t   NodeId,
					      HSA_QUEUEID QueueId)
{
	HSAint32 PollFd = 0;
	device_status_t status = EnableDebugTrapWithPollFd(NodeId,
							       QueueId,
							       &PollFd);
...
device_status_t Debug::DisableDebugTrap(uint32_t NodeId)
{
...
device_status_t Debug::SetWaveLaunchTrapOverride(
					uint32_t NodeId,
...
device_status_t Debug::SetWaveLaunchMode(
				uint32_t NodeId,
...
device_status_t Debug::QueueSuspend(
		uint32_t    Pid,         // IN
...
*/
device_status_t
Debug::GetKernelDebugTrapVersionInfo(
    uint32_t *Major,  //Out
    uint32_t *Minor   //Out
)
{
...
*/
/*
void Debug::GetThunkDebugTrapVersionInfo(
    uint32_t *Major,  //Out
    uint32_t *Minor   //Out
)
{
...
device_status_t Debug::GetQueueSnapshot(
		uint32_t	NodeId, //IN
		uint32_t	Pid, // IN
		bool		ClearEvents, //IN
		void		*SnapshotBuf, //IN
...
device_status_t Debug::SetAddressWatch(
		uint32_t		NodeId,
		uint32_t		Pid,
		HSA_DBG_WATCH_MODE	WatchMode,
		void			*WatchAddress,
		...
		)
{
	 ...
	 */
	if (WatchAddrMask > (uint64_t) std::numeric_limits<uint64_t>::max())
	{
		return DEVICE_STATUS_INVALID_PARAMETER;
	}
...
device_status_t Debug::ClearAddressWatch(
		uint32_t NodeId,
		uint32_t Pid,
		uint32_t WatchId
		)
...
device_status_t Debug::SetTrapHandler(HSAuint32 Node,
					     ...
					     HSAuint64 TrapBufferSizeInBytes)
{
	...
	result = device_->get_topo()->validate_nodeid(Node, &gpu_id);
	if (result != DEVICE_STATUS_SUCCESS)
		return result;
