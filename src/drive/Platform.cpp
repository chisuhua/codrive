...
GpuAgent* Platform::DiscoverGpu(HSAuint32 node_id, HsaCoreProperties* node_prop) {
  if (node_prop->NumFComputeCores == 0) {
    return NULL;
  }

  GpuAgent* gpu = new GpuAgent(node_id, node_prop);
  Runtime::runtime_singleton_->RegisterAgent(gpu);

  return gpu;
}

void Platform::RegisterLinkInfo(uint32_t node_id, uint32_t num_link) {
  ...
  if (DEVICE_STATUS_SUCCESS !=
      DEVICE->GetNodeIoLinkProperties(node_id, num_link, &links[0])) {
    return;
  }
  ...
  for (HsaIoLinkProperties io_link : links) {
    ...
    Runtime::runtime_singleton_->RegisterLinkInfo(
...
*/
  ...
  DEVICE->ReleaseSystemProperties();

  if (DEVICE->AcquireSystemProperties(&props) != DEVICE_STATUS_SUCCESS) {
    return;
  }

  Runtime::runtime_singleton_->SetLinkCount(props.NumNodes);
  ...
  for (HSAuint32 node_id = 0; node_id < props.NumNodes; node_id++) {
    ...
    if (DEVICE->GetNodeProperties(node_id, &node_prop) != DEVICE_STATUS_SUCCESS) {
      continue;
    }
...
bool Platform::Load() {
  ...
  if (DEVICE->Open() != DEVICE_STATUS_SUCCESS) {
      return false;
  }
