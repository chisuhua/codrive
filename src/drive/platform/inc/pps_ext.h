...
#include "pps.h"
#include "pps_ext_image.h"
#include "pps_ext_api_trace.h"
...
/**
 * @brief Enumeration constants added to ::status_t.
 *
 * @remark Additions to status_t
 */
enum {
  /**
   * The memory pool is invalid.
   */
  ERROR_INVALID_MEMORY_POOL = 40,

  /**
   * Agent accessed memory beyond the maximum legal address.
   */
  ERROR_MEMORY_APERTURE_VIOLATION = 41,

  /**
   * Agent executed an invalid shader instruction.
   */
  ERROR_ILLEGAL_INSTRUCTION = 42,
};
 ...
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p type is NULL.
 */
status_t HSA_API hsa_amd_coherency_get_type(device_t agent,
 ...
 * @retval ::ERROR_INVALID_AGENT The agent is invalid.
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p type is invalid.
 */
status_t HSA_API hsa_amd_coherency_set_type(device_t agent,
                                                hsa_amd_coherency_type_t type);
 ...
 */
status_t HSA_API
    hsa_amd_profiling_set_profiler_enabled(queue_t* queue, int enable);
 ...
 * initialized.
 *
 * @retval ::ERROR_OUT_OF_RESOURCES Failed on allocating resources
 ...
 * @retval ::ERROR_INVALID_AGENT The agent is invalid.
 *
 * @retval ::ERROR_INVALID_SIGNAL The signal is invalid.
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p time is NULL.
 */
status_t HSA_API hsa_amd_profiling_get_dispatch_time(
    device_t agent, signal_t signal,
 ...
 * @retval ::ERROR_INVALID_ARGUMENT @p time is NULL.
 */
status_t HSA_API hsa_amd_profiling_get_async_copy_time(
    signal_t signal, hsa_amd_profiling_async_copy_time_t* time);
 ...
 * @retval ::ERROR_INVALID_AGENT The agent is invalid.
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p system_tick is NULL;
 */
status_t HSA_API
    hsa_amd_profiling_convert_tick_to_system_domain(device_t agent,
                                                    uint64_t agent_tick,
 ...
 * contains duplicates.
 */
status_t HSA_API hsa_amd_signal_create(signal_value_t initial_value, uint32_t num_consumers,
 ...
 * initialized.
 *
 * @retval ::ERROR_INVALID_SIGNAL signal is not a valid signal_t
 *
 * @retval ::ERROR_INVALID_ARGUMENT handler is invalid (NULL)
 *
 * @retval ::ERROR_OUT_OF_RESOURCES The HSA runtime is out of
 * resources or blocking signals are not supported by the HSA driver component.
 *
 */
status_t HSA_API
    hsa_amd_signal_async_handler(signal_t signal,
 ...
 */
uint32_t HSA_API
    hsa_amd_signal_wait_any(uint32_t signal_count, signal_t* signals,
                            signal_condition_t* conds,
                            signal_value_t* values, uint64_t timeout_hint,
 ...
 * initialized.
 *
 * @retval ::ERROR_INVALID_QUEUE @p value is NULL or @p attribute <
 * HSA_EXT_AGENT_INFO_IMAGE_1D_MAX_ELEMENTS or @p attribute >
 * HSA_EXT_AGENT_INFO_IMAGE_ARRAY_MAX_LAYERS.
 *
 */
status_t HSA_API hsa_amd_image_get_info_max_dim(device_t agent,
 ...
 * initialized.
 *
 * @retval ::ERROR_INVALID_QUEUE @p queue is NULL or invalid.
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p num_cu_mask_count is not
 * multiple of 32 or @p cu_mask is NULL.
 *
 * @retval ::ERROR failed to call thunk api
 *
 */
status_t HSA_API hsa_amd_queue_cu_set_mask(const queue_t* queue,
 ...
 * returns a status other than ::SUCCESS for a particular iteration,
 ...
 */
status_t HSA_API hsa_amd_agent_iterate_memory_pools(
    device_t agent,
 ...
 * initialized.
 *
 * @retval ::ERROR_OUT_OF_RESOURCES No memory is available.
 *
 * @retval ::ERROR_INVALID_MEMORY_POOL The memory pool is invalid.
 *
 ...
 *
 */
status_t HSA_API hsa_amd_memory_pool_free(void* ptr);
 ...
 *
 * @retval ::SUCCESS The function has been executed successfully. The
 ...
 * @retval ::ERROR_INVALID_AGENT The agent is invalid.
 *
 * @retval ::ERROR_INVALID_SIGNAL @p completion_signal is invalid.
 *
...
// FIXME remote dir
status_t HSA_API
    hsa_amd_memory_async_copy(void* dst, device_t dst_agent, const void* src,
                              device_t src_agent, size_t size, 
                              uint32_t num_dep_signals,
                              const signal_t* dep_signals,
...
*/
status_t HSA_API hsa_amd_memory_async_copy_rect(
    const hsa_pitched_ptr_t* dst, const hsa_dim3_t* dst_offset, const hsa_pitched_ptr_t* src,
    const hsa_dim3_t* src_offset, const hsa_dim3_t* range, device_t copy_agent,
 ...
 *
 */
status_t HSA_API hsa_amd_agent_memory_pool_get_info(
    device_t agent, hsa_amd_memory_pool_t memory_pool,
 ...
 * initialized.
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p num_agents is 0, or @p agents
 ...
 */
status_t HSA_API
    hsa_amd_agents_allow_access(uint32_t num_agents, const device_t* agents,
 ...
 * initialized.
 *
 * @retval ::ERROR_INVALID_MEMORY_POOL One of the memory pools is
 * invalid.
 *
 ...
 * initialized.
 *
 * @retval ::ERROR_INVALID_MEMORY_POOL The destination memory pool is
 * invalid.
 *
 * @retval ::ERROR_OUT_OF_RESOURCES There is a failure in
 * allocating the necessary resources.
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p flags is not 0.
 */
status_t HSA_API hsa_amd_memory_migrate(const void* ptr,
 ...
 * allocating the necessary resources.
 *
 * @retval ::ERROR_INVALID_AGENT One or more agent in @p agents is
 * invalid.
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p size is 0 or @p host_ptr or
 * @p agent_ptr is NULL or @p agents not NULL but @p num_agent is 0 or @p agents
 * is NULL but @p num_agent is not 0.
 */
status_t HSA_API hsa_amd_memory_lock(void* host_ptr, size_t size,
 ...
 *
 * @retval ::ERROR_INVALID_MEMORY_POOL @p pool is invalid or not owned
 * by a CPU agent.
 *
 * @retval ::ERROR_INVALID_ARGUMENT @p size is 0 or @p host_ptr or
 * @p agent_ptr is NULL or @p agents not NULL but @p num_agent is 0 or @p agents
 ...
 */
status_t HSA_API hsa_amd_memory_lock_to_pool(void* host_ptr, size_t size, device_t* agents,
 ...
 * initialized.
 */
status_t HSA_API hsa_amd_memory_unlock(void* host_ptr);
 ...
 * initialized.
 *
 * @retval ERROR_INVALID_ARGUMENT @p ptr is NULL or
 * not 4 bytes aligned
 *
 * @retval ERROR_INVALID_ALLOCATION if the given memory
 ...
 *
 * @retval SUCCESS if successfully mapped
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_OUT_OF_RESOURCES if there is a failure in allocating
 * necessary resources
 *
 * @retval ERROR_INVALID_ARGUMENT all other errors
 */
status_t HSA_API hsa_amd_interop_map_buffer(uint32_t num_agents,
                                        device_t* agents,
                                        int interop_handle,
                                        uint32_t flags,
                                        size_t* size,
 ...
 */
status_t HSA_API hsa_amd_interop_unmap_buffer(void* ptr);
 ...
 *
 * @retval SUCCESS Image created successfully
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_OUT_OF_RESOURCES if there is a failure in allocating
 * necessary resources
 *
 * @retval ERROR_INVALID_ARGUMENT Bad or mismatched descriptor,
 * null image_data, or mismatched access_permission.
 */
status_t HSA_API hsa_amd_image_create(
    device_t agent,
 ...
 * May be NULL.
 *
 * @retval SUCCESS Info retrieved successfully
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_OUT_OF_RESOURCES if there is a failure in allocating
 * necessary resources
 *
 * @retval ERROR_INVALID_ARGUMENT NULL in @p ptr or @p info.
 */
status_t HSA_API hsa_amd_pointer_info(void* ptr,
 ...
 *
 * @retval SUCCESS @p userdata successfully stored.
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_OUT_OF_RESOURCES if there is a failure in allocating
 * necessary resources
 *
 * @retval ERROR_INVALID_ARGUMENT @p ptr is not known to ROCr.
 */
status_t HSA_API hsa_amd_pointer_info_set_userdata(void* ptr,
                                                       void* userdata);
 ...
 * allocation.
 *
 * @retval SUCCESS allocation is prepared for interprocess sharing.
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_OUT_OF_RESOURCES if there is a failure in allocating
 * necessary resources
 *
 * @retval ERROR_INVALID_ARGUMENT @p ptr does not point to the
 * first byte of an allocation made through ROCr, or len is not the full length
 * of the allocation or handle is NULL.
 */
status_t HSA_API hsa_amd_ipc_memory_create(void* ptr, size_t len,
 ...
 *
 * @retval SUCCESS if memory is successfully imported.
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_OUT_OF_RESOURCES if there is a failure in allocating
 ...
 *
 * @retval SUCCESS if @p mapped_ptr was imported with
 * hsa_amd_ipc_memory_attach.
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_INVALID_ARGUMENT @p mapped_ptr was not imported
 * with hsa_amd_ipc_memory_attach.
 */
status_t HSA_API hsa_amd_ipc_memory_detach(void* mapped_ptr);
 ...
 * signal.
 *
 * @retval SUCCESS @p handle is ready to use for interprocess sharing.
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_OUT_OF_RESOURCES if there is a failure in allocating
 * necessary resources
 *
 * @retval ERROR_INVALID_ARGUMENT @p signal is not a valid signal
 * created with attribute HSA_AMD_SIGNAL_IPC or handle is NULL.
 */
status_t HSA_API hsa_amd_ipc_signal_create(signal_t signal, hsa_amd_ipc_signal_t* handle);
 ...
 *
 * @retval SUCCESS if the signal is successfully imported.
 *
 * @retval ERROR_NOT_INITIALIZED if HSA is not initialized
 *
 * @retval ERROR_OUT_OF_RESOURCES if there is a failure in allocating
 * necessary resources
 *
 * @retval ERROR_INVALID_ARGUMENT @p handle is not valid.
 */
status_t HSA_API hsa_amd_ipc_signal_attach(const hsa_amd_ipc_signal_t* handle,
                                               signal_t* signal);
 ...
 * necessary resources
 */
status_t HSA_API hsa_amd_register_deallocation_callback(void* ptr,
 ...
 */
status_t HSA_API hsa_amd_deregister_deallocation_callback(void* ptr,
                                                      hsa_amd_deallocation_callback_t callback);
