...
class Agent {
 public:
  // @brief Convert agent object into device_t.
  ...
  // @brief Convert agent object into const device_t.
  // @param [in] agent Pointer to an agent.
  // @retval const device_t
  ...
  enum DeviceType { kGpu = 0, kCpu = 1, kUnknownDevice = 2 };
  ...
  // @retval SUCCESS The memory copy is finished and successful.
  // TODO remote DMA_SUA
  virtual status_t DmaCopy(void* dst, core::IAgent& dst_agent,
  ...
  virtual status_t IterateCache(status_t (*callback)(ICache* cache, void* data),
                                    void* data) const = 0;
  ...
  // attribute.
  virtual status_t GetInfo(hsa_agent_info_t attribute,
                               void* value) const = 0;

  // @brief Returns an array of regions owned by the agent.
  virtual const std::vector<const core::IMemoryRegion*>& regions() const = 0;
