...
{
    void* ret = NULL;
    const device_status_t status = DEVICE->AllocMemory(node_id, size, flag, &ret);
...
void MemoryRegion::FreeKfdMemory(void* ptr, size_t size)
{
    ...
    //device_status_t status = DEVICE->FreeMemory(ptr, size);
    //assert(status == DEVICE_STATUS_SUCCESS);
    DEVICE->FreeMemory(ptr, size);
}
...
bool MemoryRegion::RegisterMemory(void* ptr, size_t size, const HsaMemFlags& MemFlags)
{
    ...
    const device_status_t status = DEVICE->RegisterMemoryWithFlags(ptr, size, MemFlags);
    return (status == DEVICE_STATUS_SUCCESS);
}

void MemoryRegion::DeregisterMemory(void* ptr) { DEVICE->DeregisterMemory(ptr); }
    ...
    HsaMemMapFlags map_flag)
{
    ...
    *alternate_va = 0;
    const device_status_t status = DEVICE->MapMemoryToGPUNodes(
...
{
    DEVICE->UnmapMemoryToGPU(const_cast<void*>(ptr));
}

MemoryRegion::MemoryRegion(bool fine_grain, bool full_profile, core::IAgent* owner,
    const HsaMemoryProperties& mem_props)
    : core::IMemoryRegion(fine_grain, full_profile, owner)
...
status_t MemoryRegion::Allocate(size_t& size, AllocateFlags alloc_flags, void** address) const
{
    if (address == NULL) {
...
status_t MemoryRegion::Free(void* address, size_t size) const
{
...
status_t MemoryRegion::IPCFragmentExport(void* address) const
{
    if (!fragment_allocator_.discardBlock(address)) return ERROR_INVALID_ALLOCATION;
    return SUCCESS;
}

status_t MemoryRegion::GetInfo(region_info_t attribute,
    void* value) const
{
    switch (attribute) {
...
status_t MemoryRegion::GetPoolInfo(hsa_amd_memory_pool_info_t attribute,
    void* value) const
{
    switch (attribute) {
...
status_t MemoryRegion::GetAgentPoolInfo(
...
status_t MemoryRegion::AllowAccess(uint32_t num_agents,
    core::IAgent** agents,
    const void* ptr, size_t size) const
{
    ...
    if (whitelist_nodes.size() == 0 && IsSystem()) {
        ...
        // Remove old mappings.
        MemoryRegion::MakeKfdMemoryUnresident(ptr);
        return SUCCESS;
    }
...

    {
        ...
        uint64_t alternate_va = 0;
        if (!MemoryRegion::MakeKfdMemoryResident(
...
status_t MemoryRegion::CanMigrate(const MemoryRegion& dst,
    bool& result) const
{
...
status_t MemoryRegion::Migrate(uint32_t flag, const void* ptr) const
{
    // TODO: not implemented yet.
    return ERROR_OUT_OF_RESOURCES;
}

status_t MemoryRegion::Lock(uint32_t num_agents, core::IAgent** agents,
...
*/
        ...
        MemoryRegion::DeregisterMemory(host_ptr);
        return ERROR_OUT_OF_RESOURCES;
    }
...
status_t MemoryRegion::Unlock(void* host_ptr) const
{
    if (!IsSystem()) {
        return ERROR;
    }
...
status_t MemoryRegion::AssignAgent(void* ptr, size_t size,
...
void* MemoryRegion::BlockAllocator::alloc(size_t request_size, size_t& allocated_size) const
{
    ...
    status_t err = region_.Allocate(
        bsize, core::IMemoryRegion::AllocateRestrict | core::IMemoryRegion::AllocateDirect, &ret);
    if (err != SUCCESS) {
        // throw ::hcs::hsa_exception(err, "MemoryRegion::BlockAllocator::alloc failed.");
        throw co_exception(err, "MemoryRegion::BlockAllocator::alloc failed.");
    }
