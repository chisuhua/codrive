...
class IBlit {
 public:
  ...
  ///
  /// @param device Pointer to the device that will execute the blit commands.
  ///
  /// @return status_t
  virtual status_t Initialize(const core::IAgent& device) = 0;
  ...
  ///
  /// @param device Agent passed to Initialize.
  ///
  /// @return status_t
  virtual status_t Destroy(const core::IAgent& device) = 0;
  ...
  virtual status_t SubmitLinearCopyCommand(
      ...
      std::vector<signal_t>& dep_signals, signal_t& out_signal) = 0;
