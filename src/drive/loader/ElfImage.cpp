...
#define OpenTemp(f) file_util::OpenTempFile(f);
#define CloseTemp(f) file_util::CloseTempFile(f);
...
namespace ppu {
  namespace elf {
    ...
    public:
      ElfSymbol(ElfSymbolTable* symtab, Buffer &data, size_t index);
    ...
    private:
      // Elf_Sym* Sym() { return edata.get<GElf_Sym*>(eindex); }
...
*/
  ...
  namespace elf {
    ...
    bool GElfSymbolTable::push(const char* name, GElfStringTable* strtab)
    {
      ...
      this->strtab = strtab;
      // TODO schi hack , change sh_info to 1
      // if (!GElfSection::push(name, SHT_SYMTAB, 0, strtab->getSectionIndex(), 0, 0, sizeof(Elf64_Sym))) { return false;  }
      if (!GElfSection::push(name, SHT_SYMTAB, 0, strtab->getSectionIndex(), 1, 0, sizeof(Elf64_Sym))) { return false;  }
      return true;
    }
...
/* use ELIO to read note
          ELFIO::note_section_accessor notes(elf_reader, section);
      ...
      while ((data = elf_getdata(scn, data)) != 0) {
        ...
        while (note_offset < data->d_size) {
          ...
          if (type == note->n_type) {
            ...
            if ((note_name == "AMD") || (name == note_name)) {
