...
static struct opu_gpu_cache_info kaveri_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info carrizo_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info vega10_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info raven_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info renoir_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info vega12_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info vega20_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info aldebaran_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info navi10_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info vangogh_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info navi14_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info sienna_cichlid_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info navy_flounder_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info dimgrey_cavefish_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info beige_goby_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static struct opu_gpu_cache_info yellow_carp_cache_info[] = {
	{
		/* TCP L1 Cache per CU */
		.cache_size = 16,
		.cache_level = 1,
...
static void opu_populated_cu_info_cpu(struct opu_topology_device *dev,
...
static void opu_populated_cu_info_gpu(struct opu_topology_device *dev,
		struct crat_subtype_computeunit *cu)
{
...
/* opu_parse_subtype_cu - parse compute unit subtypes and attach it to correct
 * topology device present in the device_list
 */
static int opu_parse_subtype_cu(struct crat_subtype_computeunit *cu,
	...
	list_for_each_entry(dev, device_list, list) {
		if (cu->proximity_domain == dev->proximity_domain) {
			if (cu->flags & CRAT_CU_FLAGS_CPU_PRESENT)
				opu_populated_cu_info_cpu(dev, cu);

			if (cu->flags & CRAT_CU_FLAGS_GPU_PRESENT)
				opu_populated_cu_info_gpu(dev, cu);
			break;
		}
	}
...
/* opu_parse_subtype_mem - parse memory subtypes and attach it to correct
 * topology device present in the device_list
 */
static int opu_parse_subtype_mem(struct crat_subtype_memory *mem,
				struct list_head *device_list)
{
	struct opu_mem_properties *props;
	struct opu_topology_device *dev;
...
/* opu_parse_subtype_cache - parse cache subtypes and attach it to correct
 * topology device present in the device_list
 */
static int opu_parse_subtype_cache(struct crat_subtype_cache *cache,
			struct list_head *device_list)
{
	struct opu_cache_properties *props;
	struct opu_topology_device *dev;
	uint32_t id;
...
/* opu_parse_subtype_iolink - parse iolink subtypes and attach it to correct
 * topology device present in the device_list
 */
static int opu_parse_subtype_iolink(struct crat_subtype_iolink *iolink,
					struct list_head *device_list)
{
	struct opu_iolink_properties *props = NULL, *props2;
	struct opu_topology_device *dev, *to_dev;
	uint32_t id_from;
	uint32_t id_to;
	...
	if (props && (iolink->flags & CRAT_IOLINK_FLAGS_BI_DIRECTIONAL)) {
		to_dev = opu_topology_device_by_proximity_domain(id_to);
		if (!to_dev)
			return -ENODEV;
...
/* opu_parse_subtype - parse subtypes and attach it to correct topology device
 * present in the device_list
 *	@sub_type_hdr - subtype section of crat_image
 *	@device_list - list of topology devices present in this crat_image
 */
static int opu_parse_subtype(struct crat_subtype_generic *sub_type_hdr,
				struct list_head *device_list)
{
	struct crat_subtype_computeunit *cu;
	...
	case CRAT_SUBTYPE_COMPUTEUNIT_AFFINITY:
		...
		ret = opu_parse_subtype_cu(cu, device_list);
		break;
	case CRAT_SUBTYPE_MEMORY_AFFINITY:
		mem = (struct crat_subtype_memory *)sub_type_hdr;
		ret = opu_parse_subtype_mem(mem, device_list);
		break;
	case CRAT_SUBTYPE_CACHE_AFFINITY:
		cache = (struct crat_subtype_cache *)sub_type_hdr;
		ret = opu_parse_subtype_cache(cache, device_list);
		break;
	...
	case CRAT_SUBTYPE_IOLINK_AFFINITY:
		...
		ret = opu_parse_subtype_iolink(iolink, device_list);
		break;
	default:
...
/* opu_parse_crat_table - parse CRAT table. For each node present in CRAT
 * create a opu_topology_device and add in to device_list. Also parse
 * CRAT subtypes and attach it to appropriate opu_topology_device
 *	@crat_image - input image containing CRAT
 *	@device_list - [OUT] list of opu_topology_device generated after
 *		       parsing crat_image
 *	@proximity_domain - Proximity domain of the first device in the table
 *
 *	Return - 0 if successful else -ve value
 */
int opu_parse_crat_table(void *crat_image, struct list_head *device_list,
			 uint32_t proximity_domain)
{
	...
	for (node_id = 0; node_id < num_nodes; node_id++) {
		top_dev = opu_create_topology_device(device_list);
		if (!top_dev)
			break;
	...
	while ((char *)sub_type_hdr + sizeof(struct crat_subtype_generic) <
		...
		if (sub_type_hdr->flags & CRAT_SUBTYPE_FLAGS_ENABLED) {
			ret = opu_parse_subtype(sub_type_hdr, device_list);
			if (ret)
				break;
		}
...
err:
	if (ret)
		opu_release_topology_device_list(device_list);

	return ret;
}

/* Helper function. See opu_fill_gpu_cache_info for parameter description */
static int fill_in_l1_pcache(struct crat_subtype_cache *pcache,
				struct opu_gpu_cache_info *pcache_info,
...
/* opu_fill_gpu_cache_info - Fill GPU cache info using opu_gpu_cache_info
 * tables
 *
 *	@kdev - [IN] GPU device
 ...
 */
static int opu_fill_gpu_cache_info(struct opu_dev *kdev,
			...
			int *num_of_entries)
{
	...
	*num_of_entries = 0;
	...
	/* For each type of cache listed in the opu_gpu_cache_info table,
	 * go through all available Compute Units.
	 * The [i,j,k] loop will
	 *		if opu_gpu_cache_info.num_cu_shared = 1
...
static bool opu_ignore_crat(void)
{
	bool ret;
...
/*
 * opu_create_crat_image_acpi - Allocates memory for CRAT image and
 * copies CRAT from ACPI (if available).
 * NOTE: Call opu_destroy_crat_image to free CRAT image memory
 *
 ...
 */
int opu_create_crat_image_acpi(void **crat_image, size_t *size)
{
...
/* opu_fill_cu_for_cpu - Fill in Compute info for the given CPU NUMA node
 *
 ...
 */
static int opu_fill_cu_for_cpu(int numa_node_id, int *avail_size,
				...
				struct crat_subtype_computeunit *sub_type_hdr)
{
	...
	*avail_size -= sizeof(struct crat_subtype_computeunit);
	...
	sub_type_hdr->processor_id_low = opu_numa_node_to_apic_id(numa_node_id);
...
/* opu_fill_mem_info_for_cpu - Fill in Memory info for the given CPU NUMA node
 *
 *	@numa_node_id: CPU NUMA node id
 *	@avail_size: Available size in the memory
 *	@sub_type_hdr: Memory into which compute info will be filled in
 ...
 *	Return 0 if successful else return -ve value
 */
static int opu_fill_mem_info_for_cpu(int numa_node_id, int *avail_size,
			int proximity_domain,
...
#ifdef CONFIG_X86_64
static int opu_fill_iolink_info_for_cpu(int numa_node_id, int *avail_size,
...
/* opu_create_vcrat_image_cpu - Create Virtual CRAT for CPU
 *
 *	@pcrat_image: Fill in VCRAT for CPU
 *	@size:	[IN] allocated size of crat_image.
 *		[OUT] actual size of data filled in crat_image
 */
static int opu_create_vcrat_image_cpu(void *pcrat_image, size_t *size)
{
	...
	for_each_online_node(numa_node_id) {
		if (opu_numa_node_to_apic_id(numa_node_id) == -1)
			continue;

		/* Fill in Subtype: Compute Unit */
		ret = opu_fill_cu_for_cpu(numa_node_id, &avail_size,
		...
		ret = opu_fill_mem_info_for_cpu(numa_node_id, &avail_size,
			crat_table->num_domains,
			(struct crat_subtype_memory *)sub_type_hdr);
		if (ret < 0)
			return ret;
...
#ifdef CONFIG_X86_64
		ret = opu_fill_iolink_info_for_cpu(numa_node_id, &avail_size,
				&entries,
				(struct crat_subtype_iolink *)sub_type_hdr);
		if (ret < 0)
			return ret;
...
static int opu_fill_gpu_memory_affinity(int *avail_size,
		struct opu_dev *kdev, uint8_t type, uint64_t size,
		struct crat_subtype_memory *sub_type_hdr,
		uint32_t proximity_domain,
		const struct opu_local_mem_info *local_mem_info)
{
	*avail_size -= sizeof(struct crat_subtype_memory);
	if (*avail_size < 0)
		return -ENOMEM;
...
static void opu_find_numa_node_in_srat(struct opu_dev *kdev)
{
...
/* opu_fill_gpu_direct_io_link - Fill in direct io link from GPU
 * to its NUMA node
 *	@avail_size: Available size in the memory
 *	@kdev - [IN] GPU device
 ...
 *	Return 0 if successful else return -ve value
 */
static int opu_fill_gpu_direct_io_link_to_cpu(int *avail_size,
			struct opu_dev *kdev,
			struct crat_subtype_iolink *sub_type_hdr,
			uint32_t proximity_domain)
{
	...
	*avail_size -= sizeof(struct crat_subtype_iolink);
	...
	if (kdev->pdev->dev.numa_node == NUMA_NO_NODE)
		opu_find_numa_node_in_srat(kdev);
#endif
#ifdef CONFIG_NUMA
	if (kdev->pdev->dev.numa_node == NUMA_NO_NODE)
...
static int opu_fill_gpu_xgmi_link_to_gpu(int *avail_size,
			struct opu_dev *kdev,
			struct opu_dev *peer_kdev,
			struct crat_subtype_iolink *sub_type_hdr,
			...
			uint32_t proximity_domain_to)
{
	*avail_size -= sizeof(struct crat_subtype_iolink);
	...
	sub_type_hdr->num_hops_xgmi =
		amdgpu_amdopu_get_xgmi_hops_count(kdev->kgd, peer_kdev->kgd);
	return 0;
}

/* opu_create_vcrat_image_gpu - Create Virtual CRAT for CPU
 *
 *	@pcrat_image: Fill in VCRAT for GPU
 *	@size:	[IN] allocated size of crat_image.
 *		[OUT] actual size of data filled in crat_image
 */
static int opu_create_vcrat_image_gpu(void *pcrat_image,
				      size_t *size, struct opu_dev *kdev,
				      uint32_t proximity_domain)
{
	struct crat_header *crat_table = (struct crat_header *)pcrat_image;
	struct crat_subtype_generic *sub_type_hdr;
	...
	amdgpu_amdopu_get_cu_info(kdev->kgd, &cu_info);
	 ...
	 */
	if (!opu_iommu_check_device(kdev))
	 ...
	 * private heap.
	 */
	amdgpu_amdopu_get_local_mem_info(kdev->kgd, &local_mem_info);
	sub_type_hdr = (typeof(sub_type_hdr))((char *)sub_type_hdr +
			sub_type_hdr->length);
	...
	if (local_mem_info.local_mem_size_private == 0)
		ret = opu_fill_gpu_memory_affinity(&avail_size,
	...
	sub_type_hdr = (typeof(sub_type_hdr))((char *)sub_type_hdr +
		sub_type_hdr->length);
	ret = opu_fill_gpu_cache_info(kdev, cu->processor_id_low,
				avail_size,
				&cu_info,
	...
	sub_type_hdr = (typeof(sub_type_hdr))((char *)sub_type_hdr +
		cache_mem_filled);
	ret = opu_fill_gpu_direct_io_link_to_cpu(&avail_size, kdev,
	 ...
	 * in opu_parse_subtype_iolink.
	 */
	if (kdev->hive_id) {
		for (nid = 0; nid < proximity_domain; ++nid) {
			peer_dev = opu_topology_device_by_proximity_domain(nid);
			if (!peer_dev->gpu)
				continue;
			...
			ret = opu_fill_gpu_xgmi_link_to_gpu(
...
/* opu_create_crat_image_virtual - Allocates memory for CRAT image and
 ...
 *	@kdev: Valid opu_device required if flags contain COMPUTE_UNIT_GPU
 *
 *	Return 0 if successful else return -ve value
 */
int opu_create_crat_image_virtual(void **crat_image, size_t *size,
				  int flags, struct opu_dev *kdev,
				  uint32_t proximity_domain)
{
	...
	*crat_image = NULL;
	...
	case COMPUTE_UNIT_CPU:
		...
		*size = dyn_size;
		...
		ret = opu_create_vcrat_image_cpu(pcrat_image, size);
		break;
	case COMPUTE_UNIT_GPU:
		...
		*size = VCRAT_SIZE_FOR_GPU;
		ret = opu_create_vcrat_image_gpu(pcrat_image, size, kdev,
						 proximity_domain);
		break;
...
/* opu_destroy_crat_image
 *
 *	@crat_image: [IN] - crat_image from opu_create_crat_image_xxx(..)
 *
 */
void opu_destroy_crat_image(void *crat_image)
{
	kvfree(crat_image);
}
