...
#define SVM_CWSR_BASE (SVM_USER_BASE - OPU_CWSR_TBA_TMA_SIZE)
#define SVM_IB_BASE   (SVM_CWSR_BASE - PAGE_SIZE)

static void opu_init_apertures_vi(struct opu_process_device *pdd, uint8_t id)
{
	/*
...
static void opu_init_apertures_v9(struct opu_process_device *pdd, uint8_t id)
{
...
int opu_init_apertures(struct opu_process *process)
{
	...
	while (opu_topology_enum_opu_devices(id, &dev) == 0) {
		if (!dev || opu_devcgroup_check_permission(dev)) {
		...
		pdd = opu_create_process_device_data(dev, process);
		if (!pdd) {
		 ...
		 * amdopu doesn't currently support apertures for 32 bit process
		 */
		...
		dev_dbg(opu_device, "node id %u\n", id);
		dev_dbg(opu_device, "gpu id %u\n", pdd->dev->id);
		dev_dbg(opu_device, "lds_base %llX\n", pdd->lds_base);
		dev_dbg(opu_device, "lds_limit %llX\n", pdd->lds_limit);
		dev_dbg(opu_device, "gpuvm_base %llX\n", pdd->gpuvm_base);
		dev_dbg(opu_device, "gpuvm_limit %llX\n", pdd->gpuvm_limit);
		dev_dbg(opu_device, "scratch_base %llX\n", pdd->scratch_base);
		dev_dbg(opu_device, "scratch_limit %llX\n", pdd->scratch_limit);
