...
#include "opu_kernel_queue.h"
#include "opu_priv.h"
#include "opu_pm4_opcodes.h"
#include "cik_regs.h"
#include "opu_dbgmgr.h"
#include "opu_dbgdev.h"
#include "opu_device_queue_manager.h"

static void dbgdev_address_watch_disable_nodiq(struct opu_dev *dev)
{
	dev->opu2kgd->address_watch_disable(dev->kgd);
}

static int dbgdev_diq_submit_ib(struct opu_dbgdev *dbgdev,
				...
				uint32_t *packet_buff, size_t size_in_bytes)
{
	...
	status = opu_gtt_sa_allocate(dbgdev->dev, sizeof(uint64_t),
					&mem_obj);
	...
	*rm_state = QUEUESTATE__ACTIVE_COMPLETION_PENDING;
	...
	status = amdopu_fence_wait_timeout(
			rm_state,
			QUEUESTATE__ACTIVE, 1500);

	opu_gtt_sa_free(dbgdev->dev, mem_obj);

	return status;
}

static int dbgdev_register_nodiq(struct opu_dbgdev *dbgdev)
{
	/*
...
static int dbgdev_register_diq(struct opu_dbgdev *dbgdev)
{
	...
	properties.type = OPU_QUEUE_TYPE_DIQ;
...
static int dbgdev_unregister_nodiq(struct opu_dbgdev *dbgdev)
{
...
static int dbgdev_unregister_diq(struct opu_dbgdev *dbgdev)
{
...
static int dbgdev_address_watch_nodiq(struct opu_dbgdev *dbgdev,
				      struct dbg_address_watch_info *adw_info)
{
	...
	pdd = opu_get_process_device_data(dbgdev->dev,
					adw_info->process);
	if (!pdd) {
	...
	for (i = 0; i < adw_info->num_watch_points; i++) {
		...
		pdd->dev->opu2kgd->address_watch_execute(
						dbgdev->dev->kgd,
						i,
						cntl.u32All,
						addrHi.u32All,
...
static int dbgdev_address_watch_diq(struct opu_dbgdev *dbgdev,
				    struct dbg_address_watch_info *adw_info)
{
	...
	status = opu_gtt_sa_allocate(dbgdev->dev, ib_size, &mem_obj);
	...
	for (i = 0; i < adw_info->num_watch_points; i++) {
		...
		aw_reg_add_dword =
				dbgdev->dev->opu2kgd->address_watch_get_offset(
					dbgdev->dev->kgd,
					i,
...
static int dbgdev_wave_control_diq(struct opu_dbgdev *dbgdev,
					struct dbg_wave_control_info *wac_info)
{
...
static int dbgdev_wave_control_nodiq(struct opu_dbgdev *dbgdev,
					struct dbg_wave_control_info *wac_info)
{
	int status;
	union SQ_CMD_BITS reg_sq_cmd;
	...
	/* taking the VMID for that process on the safe way using PDD */
	pdd = opu_get_process_device_data(dbgdev->dev, wac_info->process);
	...
	return dbgdev->dev->opu2kgd->wave_control_execute(dbgdev->dev->kgd,
...
int dbgdev_wave_reset_wavefronts(struct opu_dev *dev, struct opu_process *p)
{
	...
	struct opu_process_device *pdd;
	struct dbg_wave_control_info wac_info;
	int first_vmid_to_scan = dev->vm_info.first_vmid_opu;
	int last_vmid_to_scan = dev->vm_info.last_vmid_opu;
	...
	for (vmid = first_vmid_to_scan; vmid <= last_vmid_to_scan; vmid++) {
		status = dev->opu2kgd->get_atc_vmid_pasid_mapping_info
	...
	dev->opu2kgd->wave_control_execute(dev->kgd,
					reg_gfx_index.u32All,
					reg_sq_cmd.u32All);
...
void opu_dbgdev_init(struct opu_dbgdev *pdbgdev, struct opu_dev *pdev,
