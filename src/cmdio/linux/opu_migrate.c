...
{
	return (addr + adev->opu.dev->pgmap.range.start) >> PAGE_SHIFT;
}
...
svm_migrate_addr(struct amdgpu_device *adev, struct page *page)
{
	...
	return (addr - adev->opu.dev->pgmap.range.start);
}
...
static vm_fault_t svm_migrate_to_ram(struct vm_fault *vmf)
{
	...
	p = opu_lookup_process_by_mm(vma->vm_mm);
	if (!p) {
...
{
	struct opu_dev *opudev = adev->opu.dev;
	...
	if (opudev->device_info->asic_family < CHIP_VEGA10)
		return -EINVAL;
	...
	amdgpu_amdopu_reserve_system_mem(SVM_HMM_PAGE_STRUCT_SIZE(size));
...
{
	struct dev_pagemap *pgmap = &adev->opu.dev->pgmap;
